...
>   - If the check-in time is $\le$ (shiftStart + 15 minutes), the status is "On-Time".
>   - If the check-in time is $>$ (shiftStart + 15 minutes), the status is "Late".
...
This governs thhe creation and approval workflows for absences.
...
>   - **Default Status**: Everly newly and successfully saved leave or permission request always has an initial state of **Pending**.
>   - **Approval Authorization**: The action of changing a status to **Approved** or **Rejected** requires the system to verify the user's role as an authorized Approver.
...
>   - **Approval Idempotency**: If an *Approver* attempts to approve or reject a document that is *already* in an **Approved** or **Rejected** state, the system will safely handle the execution without causing a system error (returning HTTP 200/204 or an informative message) and will not corrupt historical data.
